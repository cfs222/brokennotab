...
class Main extends Route {
    ...
    const template: MenuItemConstructorOptions[] = UMenu.filterTemplate([
      ...
      },
      {
        ...
        submenu: [
          ...
          {
            label:
              keys && keys.hasNote && keys.isEditorEditing
                ? "Stop Editing"
                : "Edit",
            ...
            enabled:
              keys &&
              keys.hasNote &&
              !keys.isEditorSplitView &&
              !keys.isMultiEditorEditing,
            click: () => this.win.webContents.send("note-edit-toggle"),
          },
          {
            label:
              keys && keys.hasNote && keys.isTagsEditing
                ? "Stop Editing Tags"
                : "Edit Tags",
          ...
          {
            label:
              keys && keys.hasNote && keys.isAttachmentsEditing
                ? "Stop Editing Attachments"
                : "Edit Attachments",
            accelerator: "CmdOrCtrl+Shift+A",
            enabled: keys && keys.hasNote && !keys.isMultiEditorEditing,
            click: () =>
              this.win.webContents.send("note-edit-attachments-toggle"),
          },
          {
            type: "separator",
          },
          {
            label:
              keys && keys.hasNote && keys.isNoteFavorited
                ? "Unfavorite"
                : "Favorite",
      ...
      },
      {
        ...
        submenu: [
          ...
          },
          {
            ...
            click: () =>
              this.win.webContents.send("multi-editor-select-invert"),
          },
          {
      ...
      },
      {
        ...
        submenu: [
          ...
          { type: "separator" },
          {
            ...
            click: () =>
              shell.openExternal(`${pkg.homepage}/blob/master/CHANGELOG.md`),
          },
          {
            label: "View License",
            click: () =>
              shell.openExternal(`${pkg.homepage}/blob/master/LICENSE`),
          },
          { type: "separator" },
          {
            role: "toggledevtools",
  ...
  __printPDF = (event: Event, options: PrintOptions) => {
    ...
    if (options.html) {
      win.loadURL(
        `data:text/html;base64,${Buffer.from(options.html).toString("base64")}`
      ); //FIXME: https://github.com/electron/electron/issues/18093
    } else if (options.src) {
