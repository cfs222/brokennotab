...
class Shortcuts extends Component<{ container: IMain }, {}> {
  ...
  __selectAll = () => {
    ...
    if ($preview.length) {
      ...
      // Add this check to ensure $preview[0] exists:
      if (selection && selection.selectAllChildren && $preview[0]) {
        selection.selectAllChildren($preview[0]);
      }
    } else {
      // Select editor
  ...
  __editorSave = () => {
    if (
      !this.props.container.editor.isEditing() ||
      this.props.container.editor.isSplit()
    )
      return null;
  ...
  __editorsEscape = () => {
    if (
      this.props.container.attachments.isEditing() ||
      this.props.container.tags.isEditing() ||
      this.props.container.quickPanel.isOpen()
    )
      return null;

    if (this.props.container.multiEditor.isEditing())
      return this.props.container.multiEditor.selectClear();

    if (this.props.container.editor.isSplit())
      return this.props.container.editor.toggleSplit(false);

    if (this.props.container.editor.isEditing())
      return this.props.container.editor.toggleEditing(false);
  ...
  __noteMoveToTrash = () => {
    if (
      this.props.container.editor.isEditing() ||
      this.props.container.multiEditor.isEditing() ||
      this.props.container.note.isDeleted()
    )
      return null;
  ...
  __searchPreviousStrict = () => {
    if (
      ...
      !this.props.container.search.hasFocus()
    )
      return null;
