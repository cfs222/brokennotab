...
class ContextMenu extends Component<{ container: IMain }, {}> {
  ...
  attachment;
  note;
  tag; // Globals pointing to the current element/attachment/note/tag object
      ...
      [
        {
          label: "Open",
          click: () =>
            this.props.container.attachment.openInApp(this.attachment),
        },
        {
        ...
        },
        {
          label: "Copy",
          click: () =>
            this.props.container.clipboard.set(this.attachment.fileName),
        },
        {
          type: "separator",
        },
        ...
        },
        {
          label: "Delete",
          click: () =>
            this.props.container.note.removeAttachment(
              undefined,
              this.attachment
            ),
        },
    ...
    this._makeMenu(
      ...
      [
        ...
        },
        {
          ...
          click: () => {
            const title = this.note
              ? this.props.container.note.getTitle(this.note)
              : path.parse($(this.ele).data("filepath")).name; // Maybe we are linking to a non-existent note
        ...
        },
        {
          label: "Favorite",
          click: () =>
            this.props.container.note.toggleFavorite(this.note, true),
        },
        {
          label: "Unfavorite",
          click: () =>
            this.props.container.note.toggleFavorite(this.note, false),
        },
        {
          type: "separator",
        },
        ...
        },
        {
          label: "Restore",
          click: () =>
            this.props.container.note.toggleDeleted(this.note, false),
        },
        {
    ...
    this._makeMenu(
      (x, y) =>
        !this._getItem(
          x,
          y,
          '.attachment, .monaco-editor, .note, .popover-note-tags-list .tag, .sidebar .tag, .preview .tag, .tag[data-tag="__TRASH__"]'
        )
    );
  };
  ...
  updateNoteMenu = (items: MenuItem[]) => {
    ...
    const isFavorited = this.props.container.note.isFavorited(this.note),
      ...
      isTemplate = !!this.props.container.note.getTags(
        this.note,
        TagSpecials.TEMPLATES
      ).length;
  ...
  updateTagMenu = (items: MenuItem[]) => {
    ...
    const hasChildren = this.props.container.tag.hasChildren(this.tag),
      isCollapsed =
        hasChildren && this.props.container.tag.isCollapsed(this.tag),
