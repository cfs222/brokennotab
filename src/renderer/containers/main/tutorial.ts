...
class Tutorial extends Container<TutorialState, MainCTX> {
  ...
  import = async () => {
    ...
    try {
      ...
      Dialog.alert(
        "Failed to import the tutorial notes, please report the issue"
      );
    }
  };
  ...
  dialog = () => {
    const choice = Dialog.choice(
      "Do you want to browse the tutorial online or do you want to import all the notes?",
      ["Cancel", "Import Notes", "Online Tutorial"]
    );
