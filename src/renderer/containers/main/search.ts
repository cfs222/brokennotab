...
class Search extends Container<SearchState, MainCTX> {
  ...
  } = {}; // So that multiple subsearches are isolated from each other (middlebar, quick open...)
  ...
  _filterAttachmentsByQuery = (
    attachments: AttachmentObj[],
    query: string
  ): AttachmentObj[] => {
    return attachments.filter((attachment) =>
      this._isAttachmentMatch(attachment, query)
    );
  };
  ...
  _filterNotesByQuery = (
    ...
    const tokensRe = _.escapeRegExp(query)
      .split(Config.search.tokenizer)
      .map((token) => new RegExp(token, "i"));

    return notes.filter((note) =>
      this._isNoteMatch(note, filterContent, query, tokensRe)
    );
  };
  ...
  _searchBy = (
    ...
    prev.query = query;
    const state = (prev.state = _.pick(this.ctx.state, [
      "notes",
      "sorting",
      "tags",
      "tag",
    ...
    if (prevNotes && prevState) {
      if (
        ...
        prevState.tags === state.tags &&
        prevState.tag === state.tag
      ) {
        // Simple reordering
        ...
        isShallowEqual(prevState, state)
      ) {
        // Sub-search

        return (prev.notes = this._filterNotesByQuery(
          prevNotes,
          filterContent,
          query
        ));
    ...
    const notesByTag = this.ctx.tag.getNotes(tag),
      notesByQuery = !query
        ? notesByTag
        : this._filterNotesByQuery(notesByTag, filterContent, query),
  ...
  focus = () => {
    ...
    // FIX: Explicit type assertion for broken EleLoose type
    const input = $input[0] as HTMLElement | undefined;
  ...
  hasFocus = (): boolean => {
    ...
    // FIX: Handle undefined case and ensure both sides are Element | null
    if (!$input.length) return false;

    const input = $input[0] || null; // Converts undefined to null for valid comparison
