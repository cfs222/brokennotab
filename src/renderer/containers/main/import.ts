...
class Import extends Container<ImportState, MainCTX> {
  ...
  _importEnex = async (filePath: string) => {
    ...
    remote.dialog.showErrorBox(
      "ENEX Import Unavailable",
      "Evernote (.enex) import is temporarily disabled due to a broken dependency.\n\n" +
        'The "enex-dump" library can no longer be installed because its dependencies have been removed from GitHub.\n\n' +
        "You can still import Markdown files (.md, .txt) normally."
    );
  };
  ...
  _importMarkdown = async (filePath: string, importTag: string) => {
    ...
    const contentNext = Metadata.set(content, metadata),
      ...
      { filePath: filePathNext } = await Path.getAllowedPath(
        notesPath,
        baseName
      );
