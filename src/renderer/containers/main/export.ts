...
class Export extends Container<ExportState, MainCTX> {
  ...
  _getResource = _.memoize(
    async (resource: string, options = { minify: true }) => {
  ...
  _getResources = async (resources: string[], options = { minify: true }) => {
    const contents = await Promise.all(
      resources.map((resource) => this._getResource(resource, options))
    );
  ...
  renderers = {
    html: async (
      ...
      if (options.metadata) {
        metadata.push(
          `<meta name="metadata:tags" content="${this.ctx.note
            .getTags(note)
            .join(", ")}">`,
          `<meta name="metadata:attachments" content="${this.ctx.note
            .getAttachments(note)
            .join(", ")}">`,
          ...
          `<meta name="metadata:created" content="${this.ctx.note
            .getCreated()
            .toISOString()}">`,
          `<meta name="metadata:modified" content="${this.ctx.note
            .getModified()
            .toISOString()}">`
        );
      }
      ...
      if (options.base64) {
        ...
        for (let match of matches) {
          ...
          if (mime && base64) {
            html = html.replace(
              match[0],
              `<img${match[1]}src="data:${type};base64,${base64}"`
            );
          }
        }
      }
      ...
      if (options.base64) {
        ...
        const matches = stringMatches(html, re);
        for (let match of matches) {
          const filePath = /katex/i.test(match[1])
            ? __non_webpack_require__.resolve(`katex/dist/${match[1]}`)
            : `${__static}/fonts/IconFont.woff2`; // Simply using `require` won't work with WebPack //UGLY
  ...
  export = async (
    notes: NoteObj[] = this.ctx.multiEditor.getNotes(),
    renderer: Function,
    extension: string
  ) => {
    ...
    if (!renderer || !extension)
      return Dialog.alert("Invalid export configuration");
    ...
    const exportId = Math.random().toString(36).slice(-4),
      ...
      { filePath: exportPath } = await Path.getAllowedPath(
        basePath,
        exportName
      ),
    ...
    notes.forEach(async (note) => {
      ...
      attachments.forEach(async (fileName) => {
        ...
        const { filePath: attachmentPath } = await Path.getAllowedPath(
          attachmentsPath,
          fileName
        );
