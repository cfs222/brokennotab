...
const Path = {
  ...
  _checkAllowedPath(filePath: string): boolean {
    if (
      !Path._allowedPaths[filePath] ||
      Path._allowedPaths[filePath] + 5000 < Date.now()
    ) {
  ...
  async getAllowedPath(
    folderPath: string,
    baseName: string
  ): Promise<{ folderPath: string; filePath: string; fileName: string }> {
  ...
  sanitize(filePath: string): string {
    return filenamify(filePath, { replacement: " " }).replace(/#/g, " ").trim();
  },
};
